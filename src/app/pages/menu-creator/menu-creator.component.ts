...
export class MenuCreatorComponent implements OnInit, AfterViewInit {
  ...
  setHeaderForm(data = null) {
    this.headerForm = new FormGroup({
      auto_category: new FormControl(
        data && data.auto_category ? data.auto_category : false
      ),
      link: new FormControl(data && data.link ? data.link : ""),
      name: new FormControl(
        data && data.name ? data.name : "",
        data && !data.auto_category ? Validators.required : []
      ),
      name_ar: new FormControl(
        data && data.name_ar ? data.name_ar : "",
        data && !data.auto_category ? Validators.required : []
      ),
      image: new FormControl(data && data.image ? data.image : ""),
      levels_length: new FormControl(
        data && data.levels_length ? data.levels_length : 1,
        Validators.required
      ),
      level1_image: new FormControl(
        data && data.level1_image ? data.level1_image : false
      ),
      level2_image: new FormControl(
        data && data.level2_image ? data.level2_image : false
      ),
      level3_image: new FormControl(
        data && data.level3_image ? data.level3_image : false
      ),
      level3_items_spacing: new FormControl(
        data && data.level3_items_spacing ? data.level3_items_spacing : "20"
      ),
      level3_items_spacing_metric: new FormControl(
        data && data.level3_items_spacing_metric
          ? data.level3_items_spacing_metric
          : "px"
      ),
      menu_padding: new FormControl(
        data && data.menu_padding ? data.menu_padding : "2"
      ),
      menu_padding_metric: new FormControl(
        data && data.menu_padding_metric ? data.menu_padding_metric : "rem"
      ),
      level1_image_dimentions: new FormControl(
        data && data.level1_image_dimentions
          ? data.level1_image_dimentions
          : "300"
      ),
      level1_image_dimentions_metric: new FormControl(
        data && data.level1_image_dimentions_metric
          ? data.level1_image_dimentions_metric
          : "px"
      ),
      menu_fixed_width: new FormControl(
        data && data.menu_fixed_width ? data.menu_fixed_width : "99"
      ),
      menu_fixed_width_metric: new FormControl(
        data && data.menu_fixed_width_metric
          ? data.menu_fixed_width_metric
          : "%"
      ),
      fixed_width: new FormControl(
        data && data.fixed_width ? data.fixed_width : "30"
      ),
      fixed_width_metric: new FormControl(
        data && data.fixed_width_metric ? data.fixed_width_metric : "%"
      ),
    ...
    this.menuService.getMenu().subscribe((res) => {
      ...
      if (res.code === 200) {
        ...
        localStorage.setItem(
          "formattedJsonString",
          JSON.stringify(this.formattedJson)
        );
        this.formattedJsonString = `${localStorage.getItem(
          "formattedJsonString"
        )}`;
        ...
        if (localStorage.getItem("formattedJsonString")) {
          this.formattedJson = JSON.parse(
            localStorage.getItem("formattedJsonString")
          );
          this.formattedJsonString = `${localStorage.getItem(
            "formattedJsonString"
          )}`;
  ...
  prettyPrint(save = null) {
    var element = <HTMLTextAreaElement>(
      document.getElementById("formattedJsonString")
    );
    if (element) {
    ...
    if (save) {
      this.updating = true;
      this.menuService
        .updateMenu({ menu: this.formattedJsonString })
        .subscribe((res) => {
          if (res.code === 200) {
  ...
  fixColorCode() {
    this.formattedJson["menu_background_color"] = this.formattedJson[
      "menu_background_color"
    ].replace(" ", "");
    this.formattedJson["drop_menu_background_color"] = this.formattedJson[
      "drop_menu_background_color"
    ].replace(" ", "");
    this.formattedJson["level1_text_color"] = this.formattedJson[
      "level1_text_color"
    ].replace(" ", "");
    this.formattedJson["level1_hover_color"] = this.formattedJson[
      "level1_hover_color"
    ].replace(" ", "");
    this.formattedJson["level2_text_color"] = this.formattedJson[
      "level2_text_color"
    ].replace(" ", "");
    this.formattedJson["level2_hover_color"] = this.formattedJson[
      "level2_hover_color"
    ].replace(" ", "");
    this.formattedJson["level3_text_color"] = this.formattedJson[
      "level3_text_color"
    ].replace(" ", "");
    this.formattedJson["level3_hover_color"] = this.formattedJson[
      "level3_hover_color"
    ].replace(" ", "");
  }
  ...
  updateColorsTest() {
    this.fixColorCode();
    this.formattedJson["menu_background_color"]
      ? document.documentElement.style.setProperty(
          "--dynamic-menu-background-color",
          this.formattedJson["menu_background_color"]
        )
      : document.documentElement.style.setProperty(
          "--dynamic-menu-background-color",
    ...
    this.formattedJson["drop_menu_background_color"]
      ? document.documentElement.style.setProperty(
          "--dynamic-drob-down-menu-background-color",
          this.formattedJson["drop_menu_background_color"]
        )
      : document.documentElement.style.setProperty(
          "--dynamic-drob-down-menu-background-color",
    ...
    this.formattedJson["level1_text_color"]
      ? document.documentElement.style.setProperty(
          "--dynamic-menu-level-1-color",
          this.formattedJson["level1_text_color"]
        )
      : document.documentElement.style.setProperty(
          "--dynamic-menu-level-1-color",
    ...
    this.formattedJson["level1_hover_color"]
      ? document.documentElement.style.setProperty(
          "--dynamic-menu-level-1-hover-color",
          this.formattedJson["level1_hover_color"]
        )
      : document.documentElement.style.setProperty(
          "--dynamic-menu-level-1-hover-color",
    ...
    this.formattedJson["level2_text_color"]
      ? document.documentElement.style.setProperty(
          "--dynamic-menu-level-2-color",
          this.formattedJson["level2_text_color"]
        )
      : document.documentElement.style.setProperty(
          "--dynamic-menu-level-2-color",
    ...
    this.formattedJson["level2_hover_color"]
      ? document.documentElement.style.setProperty(
          "--dynamic-menu-level-2-hover-color",
          this.formattedJson["level2_hover_color"]
        )
      : document.documentElement.style.setProperty(
          "--dynamic-menu-level-2-hover-color",
    ...
    this.formattedJson["level3_text_color"]
      ? document.documentElement.style.setProperty(
          "--dynamic-menu-level-3-color",
          this.formattedJson["level3_text_color"]
        )
      : document.documentElement.style.setProperty(
          "--dynamic-menu-level-3-color",
    ...
    this.formattedJson["level3_hover_color"]
      ? document.documentElement.style.setProperty(
          "--dynamic-menu-level-3-hover-color",
          this.formattedJson["level3_hover_color"]
        )
      : document.documentElement.style.setProperty(
          "--dynamic-menu-level-3-hover-color",
  ...
  saveHeader() {
    if (this.headerForm.valid) {
      ...
      if (this.updateIndexHeader != null) {
        let dumbLevel2 =
          this.formattedJson.level1[this.updateIndexHeader].level2;
  ...
  sortHeaders() {
    this.formattedJson.level1.forEach(
      (item) => item.order == null && (item.order = 10000)
    );
    this.formattedJson.level1 = this.formattedJson.level1.sort((a, b) =>
      a.order < b.order ? -1 : 1
    );
  }
  ...
  saveGroup() {
    if (this.groupForm.valid) {
      ...
      if (this.updateIndexHeader != null && this.updateIndexGroup != null) {
        ...
        this.formattedJson.level1[this.updateIndexHeader].level2[
          this.formattedJson.level1[this.updateIndexHeader].level2.length - 1
        ].level3 = [];
        this.updateIndexGroup =
          this.formattedJson.level1[this.updateIndexHeader].level2.length;
      }
      this.saveChanges(true);
      this.sortGroup();
    }
  ...
  sortGroup() {
    this.formattedJson.level1[this.updateIndexHeader].level2.forEach(
      (item) => item.order == null && (item.order = 10000)
    );
    this.formattedJson.level1[this.updateIndexHeader].level2 =
      this.formattedJson.level1[this.updateIndexHeader].level2.sort((a, b) =>
        a.order < b.order ? -1 : 1
      );
  }
  ...
  saveSubCategory() {
    if (this.subCategoryForm.valid) {
      ...
      if (
        this.updateIndexHeader != null &&
        this.updateIndexGroup != null &&
        this.updateIndexSubCategory != null
      ) {
        this.formattedJson.level1[this.updateIndexHeader].level2[
          this.updateIndexGroup
        ].level3[this.updateIndexSubCategory] = this.selectedSubcategory;
      } else {
        this.formattedJson.level1[this.updateIndexHeader].level2[
          this.updateIndexGroup
        ].level3.push(this.selectedSubcategory);
      }
      this.saveChanges(true);
  ...
  sortSubCategory() {
    this.formattedJson.level1[this.updateIndexHeader].level2[
      this.updateIndexGroup
    ].level3.forEach((item) => item.order == null && (item.order = 10000));
    this.formattedJson.level1[this.updateIndexHeader].level2[
      this.updateIndexGroup
    ].level3 = this.formattedJson.level1[this.updateIndexHeader].level2[
      this.updateIndexGroup
    ].level3.sort((a, b) => (a.order < b.order ? -1 : 1));
  }
  ...
  deleteGroup() {
    ...
    this.formattedJson.level1[this.updateIndexHeader].level2.splice(
  ...
  deleteSubCategory() {
    ...
    this.formattedJson.level1[this.updateIndexHeader].level2[
      this.updateIndexGroup
    ].level3.splice(this.deleteItemIndex, 1);
    this.saveChanges(true);
  }
}
