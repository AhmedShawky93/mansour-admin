...
export class OrderDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.orderService.cancelReasons().subscribe((response: any) => {
  ...
  getOrderDetails(id) {
    ...
    this.orderService.getAvailablePickups().subscribe((response: any) => {
  ...
  submitItemUpdate() {
    this.orderService
      .updateItemPrice(this.order.id, this.currentItem.id, {
        discount_price: this.discount,
        notify_customer: this.notifyUser,
      })
      .subscribe((response: any) => {
        if (response.code === 200) {
  ...
  submitItemSerialUpdate() {
    if (this.serialNumberFormGroub.serials.valid) {
      ...
      this.allSerialNumber = this.allSerialNumber.map(
        (res) => (this.allSerialNumber = res.name)
      );
      const data = {
      ...
      this.orderService
        .updateSerial(this.order.id, data)
        .subscribe((response: any) => {
          if (response.code == 200) {
            const oneProduct = this.order.items.find(
  ...
  openEditPriceTotal(product) {
    this.discount =
      this.order.invoice.total_amount - this.order.invoice.discount;
    $("#orderChangePriceTotal").modal("show");
  }

  submitInvoiceUpdate() {
    this.orderService
      .updateInvoiceDiscount(this.order.id, {
  ...
  submitShipment() {
    ...
    shipment.pickup_date =
      moment(shipment.pickup_date).format("YYYY-MM-DD") +
      " " +
      shipment.pickup_time;
    this.orderService.createShipment(shipment).subscribe((response: any) => {
      this.stateSubmitting = false;
  ...
  customerDetails(customer) {
    ...
    if (this.order.affiliate.id !== this.order.user.id) {
      this.router.navigate(["/pages/manage-customer"], {
        queryParams: { phone: customer.phone },
      });
    }
  ...
  affiliateDetails(customer) {
    this.router.navigate(["/pages/affiliate/users"], {
      queryParams: { userId: customer.id },
    });
  }
  // customerDetails(customer) {
