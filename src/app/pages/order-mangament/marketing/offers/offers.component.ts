...
export class OffersComponent implements OnInit {
  ...
  constructor(private catService: CategoryService,
    private promoGet: PromosService, private auth: AuthService
  ) { }
  ...
  getpromo() {
    this.promoGet.getPromos()
      .subscribe((dataProme: any) => {
  ...
  getpromoId(promo) {
    this.promoGet.getPromo(promo.id)
      .subscribe((dataProme: any) => {
  ...
  changeActive(promo) {
    ...
    if (promo.active) {
      ...
      if (promo.deactivated) {
        this.promoGet.activatePromo(promo.id)
          .subscribe((data: any) => {
            promo.active = 1;
            promo.notes = "";
  ...
  submitDeactivate(promo) {
    promo.active = 0;
    this.promoGet.deactivatePromo(promo.id, { deactivation_notes: promo.notes })
      .subscribe((data: any) => {
        promo.active = 0;
