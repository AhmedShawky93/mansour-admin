...
export class EditOfferComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.activeRoute.params.subscribe((params) => {
      let id = params['id'];
      this.promoService.getPromo(id)
        .subscribe((response: any) => {
            this.promo = response.data;
            this.customers$ = concat(
                of(response.data.targets.map(customer => {
                  return {id: customer.id, name: customer.id + ": " + customer.name}
                })), // default items
            ...
            this.editForm.get('customers').setValue(response.data.targets.map((c) => c.id));
            this.promo.customers = response.data.targets.map((c) => c.id);
            this.editForm.get("editDate").setValue(new Date(this.promo.expiration_date));
          });
        })
  }
  ...
  updatePromo(promo)
  {
    ...
    promo.expiration_date = moment(this.editForm.get("editDate").value).format("YYYY-MM-DD");
    promo.customer_ids = this.editForm.get("customers").value;

    this.promoService.updatePromos(promo)
      .subscribe((response: any) => {
