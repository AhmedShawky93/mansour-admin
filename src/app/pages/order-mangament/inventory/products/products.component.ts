...
export class ProductsComponent implements OnInit {
  ...
  public lineChartLabels1: Array<any> = ['Jan', 'feb', 'mar', 'apr', 'may', 'Jun', 'jul'];
  ...
  ngOnInit() {
    ...
    this.addProductForm = new FormGroup({
      ...
      description: new FormControl('', [Validators.required, Validators.minLength(3), Validators.maxLength(250)]),
      description_ar: new FormControl('', [Validators.required, Validators.minLength(3), Validators.maxLength(250)]),
      price: new FormControl('', Validators.required),
      discount_price: new FormControl('', [Validators.max(this.product.price), Validators.min(1)]),
  ...
  getProducts() {
    this.productsService.getProducts(this.p)
      .subscribe((response: any) => {
  ...
  onimagesSelected(event, image) {
    ...
    this.uploadFile.uploadFile(this.selectImages)
      .subscribe((response: any) => {
  ...
  getProductStats() {
    this.productsService.getStats(this.currentProduct.id, {
  ...
  importExcel(event) {
    ...
    this.productsService.uploadFile(this.selectFile)
      .subscribe((response: any) => {
        // if (response.body) {
        //   this.product.image = response.body.data.name;
        //   this.product.imageUrl = response.body.data.filePath;
  ...
  submitDeactivate(product) {
    product.active = 0;
    this.productsService.deactivateProduct(product.id, { deactivation_notes: product.notes })
      .subscribe((data: any) => {
        product.active = 0;
