...
export class CategoriesComponent implements OnInit {
  ...
  constructor(private _CategoriesService: CategoryService, private uploadService: UploadFilesService, private toastrService: ToastrService) { }
  ...
  createCategory(category) {
    ...
    this._CategoriesService.createCategory(category)
  ...
  editCategory(category) {
    ...
    this.editCatForm = new FormGroup({
      ...
      name: new FormControl(category.name, [Validators.required, Validators.pattern(/[A-Za-z0-9\-\&\s]+$/)]),
      ...
      description_ar: new FormControl(category.description_ar, Validators.required),
      order: new FormControl(category.order, Validators.required),
      sub_categories: new FormArray([], [Validators.minLength(1), Validators.required])
    });

    this.editCat.sub_categories.forEach((item) => {
      this.edit_sub_categories.push(new FormGroup({
  ...
  updateCategory(category) {
    ...
    this._CategoriesService.updateCategory(category.id, category)
      .subscribe((response: any) => {
  ...
  addSubCategory(category) {
    this.sub_categories.push(new FormGroup({
  ...
  addSubCategoryEdit(category) {
    category.get('sub_categories').push(new FormGroup({
      name: new FormControl('', Validators.required),
      name_ar: new FormControl('', Validators.required),
      image: new FormControl('', Validators.required),
    }));
  ...
  changeActive(category) {
    ...
    if(category.active) {
      ...
      if(category.deactivated) {
        this._CategoriesService.activateCategory(category.id)
          .subscribe((data: any) => {
            category.active = 1;
            category.notes = "";
  ...
  submitDeactivate(category) {
    category.active = 0;
    this._CategoriesService.deactivateCategory(category.id, {deactivation_notes: category.notes})
      .subscribe((data: any) => {
        category.active = 0;
