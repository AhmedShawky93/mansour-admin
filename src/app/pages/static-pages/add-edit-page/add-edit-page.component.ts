...
export class AddEditPageComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    if (
      this.selectProductDataEdit &&
      this.selectProductDataEdit.id &&
      this.curentAction == "in"
    ) {
  ...
  addEditPages() {
    if (this.addEditPageForm.valid) {
      ...
      if (this.selectProductDataEdit) {
        this.pagesService
          .editPage(this.selectProductDataEdit.id, this.addEditPageForm.value)
          .subscribe((res) => {
            if (res.code === 200) {
        ...
        this.pagesService
          .addPage(this.addEditPageForm.value)
          .subscribe((res) => {
            if (res.code === 200) {
              this.submitting = false;
              this.dataProductEmit.emit(res.data);
  ...
  detectpageChange() {
    ...
    this.currentslug = this.slugifyPipe.transform(
      this.addEditPageForm.value.title_en
    );
  ...
  detectslugChange() {
    (document.getElementById("webslug") as HTMLInputElement).value =
      this.slugifyPipe.transform(this.addEditPageForm.value.slug);
